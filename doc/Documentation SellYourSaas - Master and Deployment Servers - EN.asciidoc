...
- Try to send an email from a deployed application.

- Try to make a backup from the internal backup tool of the deployed application (if the deployed application has such a tool).
...
[source,bash]
---------------
UPDATE llx_user set pass_crypted = '__APPPASSWORD__', email = '__APPEMAIL__' where login = 'admin' AND (pass = 'admin' OR pass_crypted = 'value_of_hashed_password_encrypted_with_password_hash');
...
page https://supervision.mysaasdomain.com/server-status
